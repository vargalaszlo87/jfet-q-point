 ...
 * Date: 2024-08-26
 *
 * Laste update: 2026-08-27 (fixing the calculations)
 *
 */
...
// models - BF259B javítva BF256B-re
const jfetModels = [
...
let V_GS = {
    Low: jfetModels[jfetIndex].params[5],
    Up: 0,
    Step: 1e-2
};
...
const jfet = {
    ...
    SolvingCurrent: (V_DD, V_GSActual, T, jfetIndex) => {
        let V_DS = 0, I_DActual = 0.0, I_DPrevius = 0, iteration = 0;
        
        jfetParameters.BETA = jfetModels[jfetIndex].params[0];
        jfetParameters.BETA_tce = jfetModels[jfetIndex].params[1] * 1e-2;
        let R_D = jfetModels[jfetIndex].params[2];
        let R_S = jfetModels[jfetIndex].params[3];
...
const e24 = {
    ...
    search: (resistor) => {
        ...
        let diff = []; // JAVÍTVA: let deklaráció hozzáadva
...
$(function() {
    ...
    $("#rangeV_DD").on('change', function() {
        ...
        updatingValue.V_GS();
        updatingValue.I_D0();
        chart.options.scales.y.max = jfet.SolvingCurrent(simulation.V_DD, 0, simulation.T, jfetIndex) * 1e3;
        chart.update();
        updatingValue.resistor();
        updatingValue.voltageGain();
        updatingValue.currentGain();
